...
import static domain.game1.game.Game.isInRange;

public class TimerMonster extends BaseMonster {
    ...
    public TimerMonster(int gridX, int gridY, int tileSize) {
    ...
    public void attack(Game game) {
        ...
        if (isInRange(this.getGridX(), this.getGridY(),player.getGridX() ,player.getGridY(), DAGGER_RANGE)) {
            ...
            if ((currentTime - lastAttackTime) > ATTACK_FREQUENCY) {
                AudioManager.playTimeStealerSound();
                game.setRemainingTime(Math.max(game.getRemainingTime() - damage, 0));
